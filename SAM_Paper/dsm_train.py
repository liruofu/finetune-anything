...
import sys
sys.path.append("//")
from dsm_dataset import PotsdamSemanticDataset, IsaidSemanticDataset
...
def train_net(SAM, device, epochs=10, batch_size=1, lr=1e-4):
    ...
    file_path = "/home/yelu/ICPR_CV/result/dsm_train_log"
    ...
    for epoch in range(epochs):
        ...
        for i, (inputs, image_path, labels, dsm) in enumerate(train_loader, 0):

            # 获取提示预测图，通道数为1
            prompt = net(image_path, model)[0].pred_sem_seg.data + torch.tensor(1)
            SAM.train()
            inputs, labels, dsm = inputs.to(device), labels.to(device),(dsm.to(device).permute(0,3,1,2))[:,0,:,:]
            #print(dsm.shape)
            ...
            outputss = SAM(inputs, prompt, labels.shape[1:],dsm)
            ...
            if (i+1) % 100 == 0:
                ...
                losss = 0
            if (i+1) % 100 == 0:
                with open(file_path + 'SAM' + '_train_' + 'log.txt', 'w') as file:
                    file.write(f'Epoch {epoch + 1}, batch {i+1}, train loss: {total_train_loss / 1000} \n')
                total_train_loss = 0

        scheduler.step()
        torch.save(SAM.state_dict(), '/home/yelu/ICPR_CV/result/dsm_train_log/sam_segb0_fine_p_d_b_m.pth')
