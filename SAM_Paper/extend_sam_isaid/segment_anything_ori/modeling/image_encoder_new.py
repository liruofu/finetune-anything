c# Copyright (c) Meta Platforms, Inc. and affiliates.
# All rights reserved.
...
class SAMAdapter(nn.Module):
    ...
    def forward(self, input_feat):
        ...
        prompt = self.mlp2(self.mlp1(input_feat))
        return prompt
...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        self.index=global_attn_indexes
        ...
        self.adapters = nn.ModuleList([SAMAdapter(embed_dim, embed_dim) for _ in range(len(self.index))])
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for j, blk in enumerate(self.blocks, 1):
            ...
            if j in self.index:
                adapter_index = self.index.index(j)
                x = blk(x) + self.adapters[adapter_index](x)
