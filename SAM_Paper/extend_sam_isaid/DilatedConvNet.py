...
class FourLayerDilatedConvNetA(nn.Module):
    def __init__(self, channels):
        ...
        # 第一层卷积，空洞率为2
        self.conv1 = nn.Conv2d(channels, channels, kernel_size=3, padding=1, dilation=2)
        # 第二层卷积，空洞率为2
        self.conv2 = nn.Conv2d(channels, channels, kernel_size=3, padding=2, dilation=2)
        # 第三层卷积，空洞率为2
        self.conv3 = nn.Conv2d(channels, channels, kernel_size=3, padding=5, dilation=2)
        # 第四层卷积，空洞率为2
        self.conv4 = nn.Conv2d(channels, channels, kernel_size=3, padding=8, dilation=2)
