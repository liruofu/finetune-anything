...
class BaseExtendSam(nn.Module):
    ...
    def multi_prompt(self, dsm ,prompt, x, all_rectangles, all_points, labels, final_masks, i):
        boxes = None
        points = None
        mask = None
        if all_rectangles[i] != []:
        ...
        sparse_embeddings, dense_embeddings = self.prompt_adapter(  # 框、点、mask提示方式
            ...
            masks=F.interpolate(dsm.unsqueeze(0).float(), (256,256), mode="bilinear", align_corners=False)
    ...
    def forward(self, img, prompt, shape,dsm):
        ...
        for i in range(1, self.num):
            ...
            self.multi_prompt(dsm, prompt, x, all_rectangles, all_points, labels, final_masks, i)

        # 将sam输出结果变为与label同大小，方便损失计算
        final_maskss = F.interpolate(final_masks, shape, mode="bilinear", align_corners=False).permute(1, 0, 2, 3)
        ##r = torch.cat([final_maskss[:, 0, :, :].unsqueeze(0),
         #F.softmax(final_maskss[:, 1:, :, :],dim=1) + g.unsqueeze(0)], dim=1)
        # 此处插入扩大感受野所使用的空洞卷积
