...
from sampaper_new.sampaper.utils  import mask_to_binary_images, draw_rectangles_and_points_on_masks,draw_rectangles_and_points_on_masks_isaid
...
#             DoubleConv(inn, inn),
#             nn.Conv2d(inn, inn, 1, 1),
#             ECANet(inn)
#         )
#         self.conv2 = nn.Sequential(
#             DoubleConv(inn, inn),
#             ECANet(inn),
#             nn.Conv2d(inn, out, 1, 1)
#         )
#         self.conv3 = nn.Sequential(
#             DoubleConv(out, out),
#             nn.Conv2d(out, out, 1, 1),
#         )
...
class BaseExtendSam(nn.Module):
    ...
    def __init__(self, class_num, ckpt_path=None, fix_img_en=False, fix_prompt_en=False, fix_mask_de=False,
        ...
        #self.net1 = net(1024, class_num-1)
        #self.net2= net(1024, class_num - 1)
    ...
    def forward(self, img, prompt, shape, g):
        ...
        # x=final_maskss[:,1:,:,:]
        # r2=self.net2(x)
        #r = torch.cat([final_masks[:, 0, :, :].unsqueeze(0), torch.sigmoid(final_masks[:, 1:, :, :]) + torch.sigmoid(g.unsqueeze(0))], dim=1)
        #r = self.net(torch.cat([torch.sigmoid(final_masks[:, 1:, :, :]) , torch.sigmoid(g.unsqueeze(0))], dim=1))
        #f= torch.cat([final_masks[:, 0, :, :].unsqueeze(0), r], dim=1)
        # final_masks[:, 1:, :, :]=torch.sigmoid(final_masks[:,1:,:,:])
        r = torch.cat([final_masks[:, 0, :, :].unsqueeze(0),
                     F.softmax(final_masks[:, 1:, :, :],dim=1) + g.unsqueeze(0)], dim=1)
        #r = torch.cat([final_masks[:, 0, :, :].unsqueeze(0),
                    #self.net1(F.softmax(final_masks[:, 1:, :, :],dim=1) )+ self.net2(g.unsqueeze(0))], dim=1)
