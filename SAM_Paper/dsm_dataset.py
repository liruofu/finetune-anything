...
class PotsdamSemanticDataset(Dataset):
    ...
    def __init__(self, root, image_set, transform=transform, with_id=False, with_mask=False, target_transform=None):
        ...
        self.dsm_dir =  os.path.join(self.root_dir, 'cropped_dsm','img_dir', image_set)
    ...
    def get_mask(self, image_id):
        ...
        if os.path.isfile(mask_path):
            ...
            mask = None
        return mask
    def get_dsm(self, image_id):
        dsm_path = os.path.join(self.dsm_dir, '0'+image_id)
        #print(dsm_path)
    ...
    def __getitem__(self, index):
        ...
        dsm = self.get_dsm(image_id)
        mask = np.array(mask)
        dsm = np.array(dsm)

        return image, image_path, mask, dsm
