...
from extend_sam_isaid import BaseExtendSam
...
def train_net(SAM, device, file_path, epochs=1, batch_size=1, lr=1e-4, batch=4):
    ...
    for epoch in range(epochs):
        total_train_loss = 0
        epoch_loss = torch.tensor(0.0, device='cuda:0', requires_grad=True)
        ...
        for i, (inputs, image_path, labels) in enumerate(train_loader, 0):
            ...
            if i % 20 == 0:
                show_masks_isaid(np.array(labels.squeeze(0).detach().cpu().numpy()), plt.gca())
                plt.show()
                show_masks_isaid(torch.argmax(outputss, dim=1).squeeze(0).detach().cpu().numpy(), plt.gca())
                plt.show()
                show_masks_isaid(prompt.squeeze(0).detach().cpu().numpy(), plt.gca())
                plt.show()
...
if __name__ == '__main__':
    ...
    # 文件存储路径
    file_path = "/home/yelu/ICPR_CV/test/isaid_segformer_10_1_M+P+B_Con/"
    start = time.time()
