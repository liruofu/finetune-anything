...
from dataset import PotsdamSemanticDataset, LovedaSemanticDataset
...
#168000
    ...
    SAM = BaseExtendSam(ckpt_path=sam_checkpoint, model_type=model_type,
                        class_num=7, fix_prompt_en=True, fix_img_en=True, fix_mask_de=True).to(device)

    SAM.load_state_dict(torch.load(
       "/home/yelu/ICPR_CV/fast/se_dual_postprocess_pots_point15_padding5/SAM_potsdam_post.pth"),strict=False)
    ...
    with torch.no_grad():
        ...
        for j, (input, path, label) in enumerate(test_loader, 0):
            ...
            result = SAM(input, prompt, label.shape[1], pred)
